...
return this.state;
    ...
    for (let i = 1; i < cb.inputs.length; i++) {
      ...
      // Enforce rollover to KSK-2017.
      if (state.hasRollover()) {
        if (!proof.isKSK2017()) {
          throw new VerifyError(block, 'invalid', 'bad-ksk-rollover', 100);
        }
      }
    }
  ...
  async getDeployments(time, prev) {
    ...
    // Rollover to KSK-2017.
    if (await this.isActive(prev, deployments.rollover))
state.rollover = true;
  ...
  setDeploymentState(state) {
    ...
    if (!this.state.hasRollover() && state.hasRollover())
      this.logger.warning('KSK-2017 rollover has been activated.');
...
continue;
      ...
      if (covenant.isClaim()) {
        ...
        if (forked !== existed) {
          throw new VerifyError(tx, 'invalid', 'bad-claim-flags', 100);
        }
...
ns.setData(data);
        ...
        case types.FINALIZE: {
          ...
          const hash = covenant.getHash(4);

          if (weak !== ns.weak || claimed !== ns.claimed) {
            throw new VerifyError(
              tx,
              'invalid',
...
return;
    ...
    if (this.db.coinCache.capacity > 0) {
      this.logger.debug(
        'Coin Cache: size=%dmb, items=%d.',
        this.db.coinCache.size / (1 << 20),
        this.db.coinCache.items
      );
    }
  }
...
return pow.bits;
    ...
    if (pow.targetReset) {
      ...
      if (time > prev.time + pow.targetSpacing * 4)
return pow.bits;
    }
  ...
  async retarget(last) {
    ...
    // This is basically a reimplementation of Zcash's
    // version of DigiShield, with very minor changes.
    // Why you should not copy/paste this if you're
    // implementing Zcash on bcoin:
    //   1. We ignore the genesis block for MTP.
    //   2. We shift right by 2 instead of divide
    //      by 4. This is different when it comes
    //      to negative numbers!
    //   3. Zcash's implementation contains a bug
    //      where the average will overflow on numbers
    //      greater than 256 bits during the averaging.
    //      This generally only occurs on testnets.
    let first = last;
