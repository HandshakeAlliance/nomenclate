...
class NomenclateDB extends EventEmitter {
  ...
  constructor(options) {
    ...
    //Status Generator
    this.statusGen = new Status(this.options);
   ...
   * @returns {Promise}
   */
  async addressHistory(hash) {
    let status = this.statusGen.new(hash);
  ...
  async nameHistory(name) {
    try {
      let status = this.statusGen.newName(name);
