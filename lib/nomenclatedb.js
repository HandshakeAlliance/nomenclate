/*!
 *index.js - Nomenclate Server for hsd
...
class NomenclateDB extends EventEmitter {
  ...
  async getAddressHashHistory(hash) {
    ...
    let hashX = Buffer.from(address.getHash(), "hex");

    const iter = this.db.iterator({
      gte: layout.o.min(hashX),
      lte: layout.o.max(hashX),
      values: true
    });
    ...
    await iter.each(async (key, raw) => {
      const [hash, txid] = layout.o.decode(key);
    ...
    for (let tx of txs) {
      let hash = Buffer.from(tx.tx_hash, "hex");
      let newtx = await this.client.getTX(hash);
      let outputIndex;

      for (let i = 0; i < newtx.outputs.length; i++) {
        if (newtx.outputs[i].address.getHash() === hash) {
          outputIndex = i;
          break;
        }
      }
      ...
      if (outputIndex) {
        let txPrefix = Buffer.from(tx.tx_hash, "hex").slice(0, 8);

        const txHashX = await this.db.get(
          layout.i.encode(txPrefix, fromU32(outputIndex))
        );

        if (txHashX) {
          const height = await this.db.get(layout.t.encode());
          let tx = {
