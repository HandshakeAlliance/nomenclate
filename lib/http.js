...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.get("/nomenclate/block/:height/headers", async (req, res) => {
      ...
      enforce(count <= MAX, "2016 is our current block max, please don't go above that");
      ...
      let [branches, root] = util.branchesAndRoot(hashes, startHeight + (count - 1));
...
//TODO revamp this entirely.
    ...
    this.get("/nomenclate/transaction/:hash/merkle/:height", async (req, res) => {
      const valid = Validator.fromRequest(req);
    ...
    this.get("/nomenclate/transaction/:height/byPosition/:pos", async (req, res) => {
      const valid = Validator.fromRequest(req);
      ...
      enforce(pos <= block.txs.length, "No transition exists at position: " + pos);

      let tx = block.txs[pos];

      if (!merkle) {
        res.text(200, tx.hash().toString("hex"));
        return;
      }
